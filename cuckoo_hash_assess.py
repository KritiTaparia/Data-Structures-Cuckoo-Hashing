...
class CuckooHash24:
	def __init__(self, init_size: int):
		...
		# self.table = [[None]*init_size for _ in range(2)]
	...
	def insert(self, key: int) -> bool:
		...
		if self.table[h0] is None:
			self.table[h0] = [key]
			return True
		elif len(self.table[h0]) != self.bucket_size:   #is not already full
			self.table[h0].append(key)
			return True
		elif self.table[h1] is None:
			self.table[h1] = [key]
			return True
		elif len(self.table[h1]) != self.bucket_size:   #is not already full
			self.table[h1].append(key)
			return True
		...
		while threshold >= 0:
			...
			if self.table[h] is None:
				self.table[h] = [key]
				return True
			if len(self.table[h]) != self.bucket_size:   #is not already full
				self.table[h].append(key)
				return True

			displace_position = self.get_rand_idx_from_bucket(h, table_index)
			displaced_value = self.table[h][displace_position]
			
			self.table[h][displace_position] = key
	...
	def lookup(self, key: int) -> bool:
		...
		if (self.table[h0] is None and self.table[h1] is None) or (key not in (self.table[h0] or []) and key not in (self.table[h1] or [])):    #if doesn't exist in both tables then return false
			return False
		return True
	...
	def delete(self, key: int) -> None:
		...
		h0 = self.hash_func(key, 0)
		h1 = self.hash_func(key, 1)
		if key in self.table[h0]:  #found in first table
			self.table[h0].remove(key)
			if self.table[h0] == []:
				self.table[h0] = None
		elif key in self.table[h1]:  #found in second table
			self.table[h1].remove(key)
			if self.table[h0] == []:
				self.table[h0] = None
